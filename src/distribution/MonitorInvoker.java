	...
	public void invoke(ClientProxy clientProxy) throws IOException, Throwable {
		...
		while (true) {
			...
			if(operation.contains("getMedicao")){
				TipoGrandeza tipo = (TipoGrandeza) msgUnmarshalled.getBody().getRequestBody().getParameters().get(0);
				...
				Medicao m = (Medicao) msgUnmarshalled.getBody().getRequestBody().getParameters().get(0);
				rObj.setmedicao(m);
